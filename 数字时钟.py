...
class Go:
    ...
    def menuf(self,event,x,y):
        ...
        if event=='WM_MBUTTONDOWN':
            self.root.withdraw()
    def about(self):
        tk.messagebox.showinfo('关于','作者：cnzb\n来源：52pojie')
    def allquit(self):
    ...
    def go(self):
        ...
        self.root.iconbitmap('数字时钟.ico')
        self.root.title('数字时钟')
        self.root.call('package','require','Winico')
        self.icon=self.root.call('winico','createfrom','数字时钟.ico')
        ...
        tk.Label(self.root,textvariable=self.var1,font=('',21)).pack(expand=1)
        tk.Label(self.root,textvariable=self.var2,font=('',25)).pack(expand=1)
        tk.Label(self.root,textvariable=self.var3,font=('Arial',45)).pack(expand=1)
        self.settime()
        panel1=tk.Frame(self.root)
        tk.Button(panel1,text='透明度',font=('',15),command=self.Attr).pack(side='left')
        tk.Button(panel1,text='多秒表',font=('',15),command=self.swt).pack(side='left')
        tk.Button(panel1,text='计次',font=('',15),command=StopWatch().stopwatch).pack(side='left')
        tk.Button(panel1,text='多倒计时/多闹钟',font=('',15),command=self.ert).pack(side='left')
        tk.Button(panel1,text='退出',font=('',15),command=self.allquit).pack(side='left')
